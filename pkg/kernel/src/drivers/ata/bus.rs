...
impl AtaBus {
    ...
    fn write_command(&mut self, drive: u8, block: u32, cmd: AtaCommand) -> storage::Result<()> {
        ...
        unsafe {
            ...
            let lba_high_nibble = bytes[3] & 0x0F;
            let high_nibble = (drive << 4) | 0xE0;
            let drive_reg = high_nibble | lba_high_nibble;
    ...
    pub(super) fn identify_drive(&mut self, drive: u8) -> storage::Result<AtaDeviceType> {
        ...
        if self.is_error() {
            warn!("ATA error: identify command error");
            self.debug();
            return Err(storage::DeviceError::InvalidOperation.into());
        }
    ...
    pub(super) fn read_pio(
        ...
        for chunk in buf.chunks_mut(2) {
            ...
            if chunk.len() > 1 {
                ...
                chunk[0] = (data & 0xFF) as u8; // only lower byte
            }
        }
